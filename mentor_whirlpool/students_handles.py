...
async def add_request(message):
    ...
    if await db.check_is_mentor(message.from_user.id):
        return
    await confirm(message, 'add_request_stud')


@bot.callback_query_handler(func=lambda call: call.data == 'add_request_stud')
async def add_request_confirmed(call):
    db = Database()
    id = await db.get_students(chat_id=call.from_user.id)

    if id and await db.get_accepted(student=id[0]['id']):
        await bot.send_message(call.from_user.id, "Вас уже обслуживает ментор")
        return
    ...
    await bot.send_message(call.from_user.id, "Добавить запрос:", reply_markup=markup)
...
async def add_request(message):
    ...
    if await db.check_is_mentor(message.from_user.id):
        return
    await confirm(message, 'readmission_work_stud')


@bot.callback_query_handler(func=lambda call: call.data == 'readmission_work_stud')
async def readmission_work_confirmed(call):
    db = Database()
    id = await db.get_students(chat_id=call.from_user.id)
    accepted = None
    if id:
    ...
    await gather(db.readmission_work(accepted[0]['id']),
                 bot.send_message(call.from_user.id, 'Вы успешно запросили доп. ментора'))
...
async def add_own_subject(message):
    ...
    if message.text in subjects:
        await gather(bot.delete_state(message.from_user.id, message.from_user.id),
                     bot.send_message(message.from_user.id, "Предмет с таким названием уже существует!"))
    else:
        ...
        await gather(bot.set_state(message.from_user.id, StudentStates.add_work_flag, message.from_user.id),
                     bot.send_message(message.from_user.id, "Введите название работы:"))
...
async def save_request(message):
    ...
    async with bot.retrieve_data(message.from_user.id, message.from_user.id) as data:
        student_dict = {'name': message.from_user.username, 'chat_id': message.from_user.id, 'subjects': [data['subject']],
    ...
    if id:
        ...
        if entered_topic in course_work_names:
            await gather(bot.delete_state(message.from_user.id, message.from_user.id),
                         bot.send_message(message.from_user.id, "Работа с таким именем уже добавлена!"))
            return
    await gather(db.add_course_work(student_dict), bot.delete_state(message.from_user.id, message.from_user.id),
                 bot.send_message(message.from_user.id, "Работа успешно добавлена! Ожидайте ответа ментора."))
...
async def remove_request(message):
    ...
    if await db.get_accepted(student=id[0]['id']):
        markup.add(types.InlineKeyboardButton(f"Удалить курсовую \"{id[0]['course_works'][0]['description']}\"",
                                              callback_data=f"confirm_delete_finale_{id[0]['id']}"))
        ...
        for course_work in student_request:
            markup.add(
                types.InlineKeyboardButton(course_work['description'],
                                           callback_data=f"confirm_delete_request_{course_work['id']}"))

        await bot.send_message(message.from_user.id, "Выберите работу, которую хотите удалить из списка запросов: ",
                               reply_markup=markup)
...
async def mentor_resume(message):
    ...
    await gather(bot.send_message(admin_chat_id, f"Пользователь @{message.from_user.username} хочет стать ментором."),
                 bot.send_message(message.from_user.id, "Ваша заявка на рассмотрении. Ожидайте ответа от администратора!"))
...
async def select_subject_callback(call):
    ...
    await bot.set_state(call.from_user.id, StudentStates.add_work_flag, call.message.from_user.id)
    async with bot.retrieve_data(call.from_user.id, call.message.from_user.id) as data:
...
async def add_own_subject_callback(call):
    await bot.set_state(call.from_user.id, StudentStates.add_own_subject_flag, call.message.from_user.id)
